...
class GoogleDrive:
  ...
  async def upload_file(self, file_path, mimeType=None):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                request = self.__service.files().create(
                    body=body, media_body=media_body, fields='id', supportsTeamDrives=True)
                response = await self.__execute_request(session, request)
